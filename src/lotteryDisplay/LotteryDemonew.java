...
public class LotteryDemonew {
	...
	public static void main(String[] args) {
		System.out.println("这是新的Demo");
		LotteryDemonew lottery = new LotteryDemonew();
	...
	public void lotteryGroups(){
		...
		if (1 == moa) {
			...
			if(0 == countZero){
				...
				/*if(1 == groupNum){				//1组手动+机选 的情况
					...
					//询问是否添加倍数
					lotTimes = lotTimes(scanner);
					int[][] multiGroup = new int[groupNum][lotteryBall.length];
					
					for(int i =0; i< groupNum; i++){
						//这个方法返回一个int[];
						multiGroup[i] = onceManualAndAutoRed(lotteryBall, countZero,i);
						printNum(multiGroup[i], lotTimes);
					//}
		...
		if( 2 == moa ){
			...
			if (groupNum<=0 || groupNum >99) {
				...
				for(int i=0; i< groupNum;i++){
					//LotteryDemonew rl = new LotteryDemonew();
					rlGroup[i] = randRedAuto();
	...
	//手动，将剩余号码进行1次机选
	public int[] onceManualAndAutoRed(int[] tempGroup, int countZero,int iCounted){
		...
		if(0 == tempGroup[tempGroup.length-1] && countZero == 1 ){
			...
			for(int i=tempGroup.length-countZero+iCounted-1; i < tempGroup.length-1; i++){		//蓝球已经进行了随机选号
				tempGroup[i] = (int) random.nextInt(redPools) + 1;
				// 在自动生成时检查是否存在重复的号码								//<-------------------重新确认
					for (int j = 0; j < i; j++) {
			...
			System.out.println("onceManualAndAutoRed中取countZero出错");
		}
		setSequence(tempGroup);
	...
	//手动，多次剩余号码机选
	/*public int[][] multiManualAndAutoRed(int[] lotteryBall,int countZero,int groupNum){
			
		int[][] multiGroups = new int[groupNum][lotteryBall.length]; 
		for(int i=0; i<groupNum;i++){
			multiGroups[i] = onceManualAndAutoRed(lotteryBall,countZero,groupNum);
			printNum(multiGroups[i], 1);						//<-----------------------打印结果正常，可以为随机数
		}
